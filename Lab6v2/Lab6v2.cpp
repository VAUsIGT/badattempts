...
int main(){
    ...
    char arr2[] = "бвгджзйклмнпрстфхцчшщБВГДЖЗЙКЛМНРПСТФХЦЧШЩ";
    char str[] = "Проверка РаБоТоСпОсОбНоСтИ";
    ...
    if(!in.is_open()){
        ...
        int j = 0;
        while(check_ch[j] != '\0'){
            check_ch[j] = tolower(check_ch[j]);// (А -> а)
            j++;
        }
        cout<<endl<<"N = "<<N<<endl;
        ...
        while(!txt.eof()){
            ...
            if(fl){ //слово подходит
                if(i < N){ //не перешли предел
                    if(end(words) == find(begin(words),end(words),word)){ //нет в массиве -> добавляем
                        words[i] = word;
                        i++;
                    }
                } else { //перешли предел
